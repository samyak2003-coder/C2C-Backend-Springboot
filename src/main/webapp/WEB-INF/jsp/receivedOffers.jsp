...
<div class="main-content">
    ...
    <div class="offers-section">
        <!-- <h2 class="section-header">Offers on your products</h2> -->
...
async function handleOfferAction(offerId, status) {
    try {
        const response = await apiCaller.put('/api/offers/' + offerId, { 
            offerId: offerId,
            status 
        });
        if (response.success) {
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        const response = await apiCaller.get('/api/offers/received');
        const offers = response.data;
        const container = document.getElementById('offers-container');
        
        if (!offers || offers.length === 0) {
            container.innerHTML = '<div class="error-message">No offers received yet</div>';
            return;
        }
        ...
        offers.forEach(offer => {
            ...
            const actions = offerData.showActions ? 
                '<div class="card-actions">' +
                '<button onclick="handleOfferAction(\'' + offerData.offerId + '\', \'ACCEPTED\')" ' +
                'class="card-button accept">Accept</button>' +
                '<button onclick="handleOfferAction(\'' + offerData.offerId + '\', \'REJECTED\')" ' +
                'class="card-button reject">Reject</button>' +
                '</div>' : '';
            
            offerBox.innerHTML = 
                '<div class="card-id">Offer ID: ' + offerData.offerId + '</div>' +
                '<div class="card-id">Product ID: ' + offerData.productId + '</div>' +
                '<div class="card-id">Buyer ID: ' + offerData.buyerId + '</div>' +
